...
export class TrailService implements OnCharacterAdd, OnDataUpdate, LogStart {
  ...
  public onCharacterAdd(character: CharacterModel): void {
    ...
    this.update(Players.GetPlayerFromCharacter(character)!);
  }

  public onDataUpdate(player: Player, directory: string): void {
  ...
  private update(player: Player): void {
    const equippedTrail = this.db.get<Maybe<string>>(player, "equippedTrail");
